...
export default function PullListings() {
  ...
  async function addRoom(store, listing) {
    ...
    const isHotelPms = listing.platform === 'webhotelier' || listing.platform === 'roomrack' || listing.platform === 'hotelizer' || listing.platform === 'cloudbeds' || listing.platform === 'loggia' || listing.platform === 'hostaway' || listing.platform === 'orange' || listing.platform === 'lodgify'
    ...
    try {
      ...
      const toInsert = selectedListings
        ...
        .map(l => ({
          name:         l.name,
          // WebHotelier, RoomRack, and Hotelizer are hotel PMSes — HostHub is short-term rental.
          room_type:    (l.platform === 'webhotelier' || l.platform === 'roomrack' || l.platform === 'hotelizer' || l.platform === 'cloudbeds' || l.platform === 'orange') ? 'hotel' : (l.platform === 'loggia' || l.platform === 'hostaway' || l.platform === 'lodgify') ? 'airbnb' : 'airbnb',
              ...
              <div key={store.id} className="pull-store-card">
                <div className="pull-store-header">
                  <div className="pull-store-identity">
                    ...
                    <span className={`badge ${store.platform === 'webhotelier' || store.platform === 'roomrack' || store.platform === 'hotelizer' || store.platform === 'cloudbeds' || store.platform === 'loggia' || store.platform === 'hostaway' || store.platform === 'orange' || store.platform === 'lodgify' ? 'badge-info' : 'badge-neutral'}`} style={{ marginLeft: '0.5rem', fontSize: '0.7rem' }}>
                      {store.platform === 'webhotelier' ? 'WebHotelier'
                        ...
                        : store.platform === 'orange' ? 'Orange PMS'
                        : store.platform === 'lodgify' ? 'Lodgify'
                        : 'HostHub'}
                    </span>
