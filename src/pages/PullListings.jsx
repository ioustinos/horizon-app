...
export default function PullListings() {
  ...
  async function addRoom(store, listing) {
    ...
    const isHotelPms = listing.platform === 'webhotelier' || listing.platform === 'roomrack' || listing.platform === 'hotelizer' || listing.platform === 'cloudbeds'
    ...
    try {
      ...
      const toInsert = selectedListings
        ...
        .map(l => ({
          name:         l.name,
          // WebHotelier, RoomRack, and Hotelizer are hotel PMSes — HostHub is short-term rental.
          room_type:    (l.platform === 'webhotelier' || l.platform === 'roomrack' || l.platform === 'hotelizer' || l.platform === 'cloudbeds') ? 'hotel' : 'airbnb',
              ...
              <div key={store.id} className="pull-store-card">
                <div className="pull-store-header">
                  <div className="pull-store-identity">
                    ...
                    <span className={`badge ${store.platform === 'webhotelier' || store.platform === 'roomrack' || store.platform === 'hotelizer' || store.platform === 'cloudbeds' ? 'badge-info' : 'badge-neutral'}`} style={{ marginLeft: '0.5rem', fontSize: '0.7rem' }}>
