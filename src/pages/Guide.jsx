    ...
    <div className="horizon-guide">
      ...
      <section id="reference" className="step-section">
        ...
        <ul>
          ...
          <li><strong>Breakfast detection (3-tier, per Lodgify's product team):</strong>
            <ol>
              <li><strong>"Breakfast" add-on on the booking</strong> — if the host (or the guest at checkout) attaches a Breakfast add-on, the booking is flagged. Important: bookings arriving through OTA channels (Airbnb, Booking.com, Vrbo) never carry add-ons.</li>
              <li><strong>"Breakfast included" amenity on the rental</strong> — the fallback for OTA traffic. The host ticks it at <em>Dashboard → Rentals → Overview → Amenities → Breakfast included</em>; every booking of that rental then counts as breakfast-included.</li>
              <li>Default false (vacation-rental baseline).</li>
            </ol>
          </li>
          ...
          <li><strong>Quirks:</strong> Lodgify bookings identify a room <em>type</em>, not a physical unit — fine for the typical whole-unit vacation rental, but a room type with several identical units cannot be split per unit (same class of limitation as WebHotelier). Horizon marks such listings during Pull Listings. <strong>Not yet verified against live data</strong> — implemented from Lodgify's docs and written answers from their product team (July 2026); first onboarding must start with a test booking.</li>
        </ul>
