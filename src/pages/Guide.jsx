  ...
  return (
    <div className="horizon-guide">
        ...
        <ul>
          <li>
            ...
            <ul>
              ...
              <li><strong>Orange PMS:</strong> a Basic Auth <strong>username + password</strong> issued by Marinet (the Orange Cloud PMS vendor, <code>apolymerou@marinet.gr</code>). Horizon discovers the hotel automatically; if one account covers several hotels, enter the username as <code>username:hotelID</code>. A demo hotel is available (username <code>wecook</code>, hotel <code>777</code>).</li>
              <li><strong>Lodgify:</strong> a single <strong>API key</strong> per host account. The host generates it themselves at <em>Settings → Integrations → Public API → "Find your API key"</em> and sends it to you. One key covers all rentals of that account. No username needed.</li>
            </ul>
          </li>
      ...
      <section id="reference" className="step-section">
        ...
        <h3>Orange PMS</h3>
        <ul>
          <li><strong>Credentials:</strong> Basic Auth username + password issued by Marinet Ltd (Orange Cloud PMS / Coral API). Horizon auto-discovers the hotel ID from the account; multi-hotel accounts disambiguate with <code>username:hotelID</code>. Demo hotel available (<code>wecook</code> / hotel <code>777</code> — "Zenith Premium Suites").</li>
          <li><strong>Breakfast detection:</strong> read directly from each reservation's <code>MealPlan</code> field. Vendor-confirmed values: <code>ROOM</code> (no breakfast) and <code>BB</code> / <code>HB</code> / <code>FB</code> / <code>AI</code> (breakfast included). Free-text English/Greek keywords are also recognized as a safety net. Default false. No per-store configuration needed.</li>
          <li><strong>Active statuses:</strong> Definite, Tentative, Guaranteed, Checked-in and Checked-out → confirmed. Cancelled and Wait-list → cancelled; a filled-in cancellation date also forces cancelled.</li>
          <li><strong>Quirks:</strong> the vendor asked us to keep API traffic around 1 call per hour — Horizon therefore fetches the whole booking window once per store per sync cycle and sorts bookings into rooms on our side (2 calls/cycle regardless of room count). Physical room numbers are first-class in Orange, so room matching uses the room number directly (like RoomRack).</li>
        </ul>

        <h3>Lodgify</h3>
        <ul>
          <li><strong>Credentials:</strong> one API key per host account, generated by the host at <em>Settings → Integrations → Public API → "Find your API key"</em>. Covers all rentals of the account. No username.</li>
          <li><strong>Breakfast detection (4-tier):</strong>
            <ol>
              <li><strong>"Breakfast" add-on on the booking</strong> — if the host (or the guest at checkout) attaches a Breakfast add-on, the booking is flagged. Bookings arriving through OTA channels (Airbnb, Booking.com, Vrbo) never carry add-ons.</li>
              <li><strong>Booking.com meal plan, recovered from the raw channel data</strong> — Lodgify's own screens drop the meal plan that Booking.com sends, but the raw payload is preserved and Horizon reads it per booking. "Breakfast included" rates flip on automatically; "no meal" and "breakfast at extra charge" rates stay off. Verified live against a real Booking.com reservation (July 2026). Airbnb bookings carry no meal plan at all, so they fall through to the next tier.</li>
              <li><strong>"Breakfast included" amenity on the rental</strong> — final fallback. The host ticks it at <em>Dashboard → Rentals → Overview → Amenities → Breakfast included</em>; every booking of that rental then counts as breakfast-included.</li>
              <li>Default false (vacation-rental baseline).</li>
            </ol>
          </li>
          <li><strong>Active statuses (strict):</strong> only <code>Booked</code> → confirmed. <code>Open</code>, <code>Tentative</code>, <code>Declined</code>, a cancellation date, or trash/deleted → cancelled.</li>
          <li><strong>Quirks:</strong> Lodgify bookings identify a room <em>type</em>, not a physical unit — fine for the typical whole-unit vacation rental, but a room type with several identical units cannot be split per unit (same class of limitation as WebHotelier). Horizon marks such listings during Pull Listings. Listings, bookings and the Booking.com meal-plan recovery were verified against a live Lodgify account (Nexus Lodges, July 2026); the add-on tier and the full order flow still need their first end-to-end run.</li>
        </ul>
